...
import com.coolwin.XYT.adapter.SearchResultAdapter;
...
public class ContactActivity extends BaseActivity implements OnClickListener, OnItemClickListener {
	...
	private void checkFriends(){
		...
		new Thread(){
			public void run() {
				try {
					CheckFriends userList = IMCommon.getIMInfo().getContactUserList(mSystemContactGlobal.getPhoneString());
		...
		new Thread(){
			public void run() {
				try {
					...
					IMJiaState status = IMCommon.getIMInfo().applyFriends(
		...
		new Thread(){
			public void run() {
				try {
					...
					IMJiaState status = IMCommon.getIMInfo().cancleFriends(uid);
					mHandler.sendEmptyMessage(GlobalParam.HIDE_PROGRESS_DIALOG);
					IMCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_STATE,status);
				} catch (IMException e) {
					e.printStackTrace();
