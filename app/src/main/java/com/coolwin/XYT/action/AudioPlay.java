...
public class AudioPlay {
	...
	private void init(BbsReplyInfo messageInfo){
		if(USE_SYSTEM_MEDIA){
			this.mMediaPlayer = new MediaPlayer();
			this.mMediaPlayer.setOnCompletionListener(mSysListenerImpl);
		}
	...
	public void play(BbsReplyInfo messageInfo, int type, boolean hasCallBack){
		Log.e(TAG, "play()");
		if(currUrl.equals(messageInfo.voiceUrl) && state){
			stop(hasCallBack);
			return;
		...
		mMessageTag = messageInfo.id;
		init(messageInfo);
		currUrl = messageInfo.voiceUrl;
		Log.e("playBbsReplyInfo","currUrl="+currUrl);
		String fileName = null;
		File file = null;
		if(currUrl.startsWith("http://")){
			fileName = FeatureFunction.generator(messageInfo.voiceUrl);
