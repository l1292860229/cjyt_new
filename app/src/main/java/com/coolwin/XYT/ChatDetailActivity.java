...
import static com.coolwin.XYT.R.id.group_ewiema_layout;
...
public class ChatDetailActivity extends BaseActivity implements OnClickListener, OnItemClickListener, OnItemLongClickListener {
	...
	private RelativeLayout mExitLayout,groupEwiemaLayout;
	private LinearLayout mChatDetailLayout,mGroupNameLayout,
	...
	private void initComponent(){
		...
		groupEwiemaLayout = (RelativeLayout)findViewById(group_ewiema_layout);
		groupEwiemaLayout.setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		case R.id.exit_chat_layout://退出登陆
			...
			break;
		case  R.id.group_ewiema_layout:
			Intent eweimaintent = new Intent(mContext, GrouperweimaActivity.class);
			eweimaintent.putExtra("title","群聊二维码");
			eweimaintent.putExtra("titlevalue",mGroupDetail.groupName);
			eweimaintent.putExtra("url","http://pre.im/jmac?sessionid="+mGroupDetail.groupId);
			eweimaintent.putExtra("usersheadsmall",mGroupDetail.mUserList.toArray(new Login[mGroupDetail.mUserList.size()]));
			startActivity(eweimaintent);
				break;
		default:
			break;
		}
			...
			if(mList.get(arg2).mType == 0){
				if(mAdapter.getIsDelete()){
					...
					if(mIsSignChat == 1){
						...
						intent.putExtra("username", mToChatLogin.phone);
						intent.putExtra("type",2);
						startActivity(intent);

					}else{
						Intent intent = new Intent(mContext, UserInfoActivity.class);
						intent.putExtra("username", mGroupDetail.mUserList.get(arg2).phone);
						intent.putExtra("type",2);
				...
				if(mAdapter.getIsDelete()){
					...
					int dj;
					try{
						dj = Integer.parseInt(login.userDj) ;
					}catch(NumberFormatException e){
						dj = 0;
					}
					if (dj==0) {
						Toast.makeText(mContext,"你还不是特级会员", Toast.LENGTH_LONG).show();
						return;
					}
		...
		new Thread(){
			public void run() {
				try {
					...
					IMJiaState statue = IMCommon.getIMInfo().modifyGroupNickName(mGroupDetail.groupId, mInputGroupName);
		...
		new Thread(){
			public void run() {
				try {
					IMCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
							mContext.getResources().getString(R.string.commit_dataing));
					IMJiaState statue = IMCommon.getIMInfo().modifyMyNickName(mGroupDetail.groupId, mInputGroupNickName);
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_GROUP_ISGET_CHECK:
				IMJiaState getRecv_status = (IMJiaState)msg.obj;
			...
			case GlobalParam.MSG_CHECK_GROUP_NAME:
				IMJiaState statue = (IMJiaState)msg.obj;
			...
			case GlobalParam.MSG_CHECK_MY_GROUP_NAME:
				IMJiaState returnStatue = (IMJiaState)msg.obj;
		...
		new Thread(){
			...
			public void run(){
				if(IMCommon.verifyNetwork(mContext)){
					try {
						Room result = IMCommon.getIMInfo().getRommInfoById(mGroupID);
	...
	private void kickPerson(final int pos){
		new Thread(){
			...
			public void run(){
				if(IMCommon.verifyNetwork(mContext)){
					try {
						IMJiaState state = IMCommon.getIMInfo().kickParticipant(mGroupID, mList.get(pos).mLogin.phone);
		...
		new Thread(){
			...
			public void run(){
				if(IMCommon.verifyNetwork(mContext)){
					...
					try {
						IMJiaState state = IMCommon.getIMInfo().inviteUsers(mGroupID, uids);
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						if(state != null && state.code == 0){
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					IMCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
							mContext.getResources().getString(R.string.send_request));
					IMJiaState stauts = null;
					if(type == 1){
						//stauts = IMCommon.getIMInfo().isPublicGroup(mRoomId,recv );
					}else if(type == 2){
						if(mIsSignChat == 1){
							stauts = IMCommon.getIMInfo().setMsg(String.valueOf(recv),mGroupID);
						}else{
							stauts = IMCommon.getIMInfo().isGetGroupMsg(mGroupID,recv );
						}
			...
			new Thread(){
				...
				public void run(){
					try {
						IMCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,
								mContext.getResources().getString(R.string.send_request));
						IMJiaState state = IMCommon.getIMInfo().deleteRoom(roomId);
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						if(state != null && state.code == 0){
			...
			new Thread(){
				...
				public void run(){
					try {
						...
						IMJiaState state = IMCommon.getIMInfo().exitRoom(roomId);
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
						if(state != null && state.code == 0){
							SQLiteDatabase db = DBHelper.getInstance(mContext).getWritableDatabase();
							RoomTable table = new RoomTable(db);
