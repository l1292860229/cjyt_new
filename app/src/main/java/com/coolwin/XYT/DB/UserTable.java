...
public class UserTable {
	...
	public static String getCreateTableSQLString() {
		if (null == mSQLCreateWeiboInfoTable) {
			...
			columnNameAndType.put(COLUMN_JOB, COLUMN_TEXT_TYPE);
			columnNameAndType.put(COLUMN_COMPANY, COLUMN_TEXT_TYPE);
			
			String primary_key = PRIMARY_KEY_TYPE + COLUMN_UID + "," + COLUMN_LOGIN_ID +"," + COLUMN_GROUP_ID + ")";
	...
	public void insert(List<Login> users, int groupId) {
		...
		for (Login user : userList) {
			...
			allPromotionInfoValues.put(COLUMN_JOB , user.job);
	...
	public void update(Login user) {
		...
		allPromotionInfoValues.put(COLUMN_COMPANY , user.company);
		try {
			mDBStore.update(TABLE_NAME, allPromotionInfoValues, COLUMN_PHONE + " = '"
					+ user.phone + "' AND " + COLUMN_LOGIN_ID + "='" + IMCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
		} catch (SQLiteConstraintException e) {
			e.printStackTrace();
		}						
	}
...
*/		
	...
	public void delete(Login user,int groupId) {
		if(user.uid == null){
			return;
		}
		mDBStore.delete(TABLE_NAME, COLUMN_UID + "='" + user.uid + "' AND " +
				COLUMN_LOGIN_ID + "='" + IMCommon.getUserId(BMapApiApp.getInstance()) + "' AND "+COLUMN_GROUP_ID+"="+groupId, null);
	}
	public void delete() {
	...
	public Login query(String uid){
		...
		try {
			String sql = "SELECT * FROM " + TABLE_NAME +
					" WHERE ((" + COLUMN_UID + "='" + uid + "' OR "+COLUMN_PHONE+"='"+uid+"') AND " + COLUMN_LOGIN_ID + "='"
			...
			if (cursor != null) {
				...
				int indexIsJob = cursor.getColumnIndex(COLUMN_JOB);
				int indexIsCompany= cursor.getColumnIndex(COLUMN_COMPANY);
				...
				user.job = cursor.getString(indexIsJob);
				user.company = cursor.getString(indexIsCompany);
			...
			if (cursor != null) {
				...
				do {
					MainSearchEntity user = new MainSearchEntity("通讯录", 100,cursor.getString(indexNickname),
							...
							cursor.getString(indexRemarkName),cursor.getString(indexPhone));
					allInfo.add(user);
