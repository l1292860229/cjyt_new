 ...
 */
	...
	public static String getCreateTableSQLString() {
		if (null == mSQLCreateWeiboInfoTable) {
			...
			columnNameAndType.put(COLUMN_RED_PACKET_TITLE, COLUMN_TEXT_TYPE);
			columnNameAndType.put(COLUMN_RED_PACKET_URL, COLUMN_TEXT_TYPE);
	...
	public void insert(List<MessageInfo> messages) {
		...
		for (MessageInfo message : messageList) {
			...
			allPromotionInfoValues.put(COLUMN_RED_PACKET_TITLE, message.redpacketTitle);
			allPromotionInfoValues.put(COLUMN_RED_PACKET_URL, message.redpacketUrl);
			allPromotionInfoValues.put(COLUMN_BID, message.bid);
			try {
	...
	public void insertThrowException(MessageInfo message) {
	...
	public boolean updateRead(){
		ContentValues allPromotionInfoValues = new ContentValues();
		allPromotionInfoValues.put(COLUMN_READ_STATE, 1);
		try {
			mDBStore.update(TABLE_NAME, allPromotionInfoValues,   COLUMN_LOGIN_ID + "='" + IMCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			return true;
		} catch (SQLiteConstraintException e) {
			e.printStackTrace();
		}
	...
	public boolean updateReadState(String id, int type, String bid){
		...
		try {
			...
			if(type == 100){
				...
				sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_READ_STATE + "=1 WHERE " + (COLUMN_FROM_UID + "='" + id + "' OR " + COLUMN_TO_ID + "='" + id) + "' AND "  + COLUMN_TYPE + "=400 AND " + COLUMN_LOGIN_ID + "='" + IMCommon.getUserId(BMapApiApp.getInstance()) + "' AND " + COLUMN_BID +"='"+bid+"'";
			}else if(type == 300){
	...
	public MessageInfo query(String tag){
		...
		try{
			...
			if (cursor != null) {
				...
				int indexRedpacketTitle = cursor.getColumnIndex(COLUMN_RED_PACKET_TITLE);
				int indexRedpacketUrl = cursor.getColumnIndex(COLUMN_RED_PACKET_URL);
				...
				message.redpacketTitle = cursor.getString(indexRedpacketTitle);
				message.redpacketUrl = cursor.getString(indexRedpacketUrl);
				message.bid = cursor.getString(indexbid);
			...
			if(type == 0){//查询所有的数据（包括单聊和群聊）
				...
				if(type == 100 && (toId!=null && !toId.equals(""))){
					querySql = "SELECT * FROM " + TABLE_NAME 
							...
							+" AND " +COLUMN_CONTENT +" like '%"+searchContent+"%' "
							+ " AND "+ COLUMN_MESSAGE_TYPE +" ="+MessageType.TEXT;
				}else if(type == 400 && (toId!=null && !toId.equals(""))){
					querySql = "SELECT * FROM " + TABLE_NAME
							+ " WHERE (" + COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "')"
							+ " AND " + COLUMN_TYPE + "=" + type
							+ " AND "+COLUMN_LOGIN_ID + "='" + IMCommon.getUserId(BMapApiApp.getInstance()) + "'"
	...
	public List<MessageInfo> query(String toId, int autoID, int type, String bid){
		...
		try {
			...
			if(type == 100){
				...
				if(autoID == -1){
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE (" + COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "')" + " AND " + COLUMN_LOGIN_ID + "='" + IMCommon.getUserId(BMapApiApp.getInstance()) + "'"
							+ " AND " + COLUMN_TYPE + "=" + type +" AND "+COLUMN_BID+"='"+bid+ "' ORDER BY rowid" + " DESC LIMIT 0," + IMCommon.LOAD_SIZE;
				}else {
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE (" + COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "')" + " AND " + COLUMN_LOGIN_ID + "='" + IMCommon.getUserId(BMapApiApp.getInstance()) + "'"
							+ " AND " + " rowid<" + autoID  + " AND " + COLUMN_TYPE + "=" + type +" AND "+COLUMN_BID+"='"+bid+ "' ORDER BY rowid" + " DESC LIMIT 0," + IMCommon.LOAD_SIZE;
				}
			}else {
				if(autoID == -1){
