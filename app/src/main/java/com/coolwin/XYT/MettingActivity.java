...
import com.coolwin.XYT.adapter.MettingAdapter;
...
import com.coolwin.XYT.widget.MeetingSearchDialog;
		...
		new Thread(){
			public void run() {
				try {
					...
					if (isExitsData) {
						mMyAlbum = IMCommon.getIMInfo().meetingList(mSelectType,page);
						if ((loadType == GlobalParam.LIST_LOAD_FIRST || loadType == GlobalParam.LIST_LOAD_REFERSH)) {
