...
public class ChatTabAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup arg2) {
		...
		if (convertView==null) {  
			...
			holder.mParentLayout = (RelativeLayout)convertView.findViewById(R.id.hsv);
		...
		if(session.type == 100){//不是房间消息
			...
			if (session.getFromId().equals("bid")) {
				holder.mHeaderView.setImageResource(R.drawable.service_icon1);
			}else{
		...
		if(session.mMessageInfo!=null){
			...
			}else if(mData.get(position).mMessageInfo.typefile == MessageType.REDPACKET){
				holder.mContentTextView.setText(mData.get(position).mMessageInfo.content);
			}else if(mData.get(position).mMessageInfo.typefile == MessageType.SHAREURL){
				holder.mContentTextView.setText("["+ mContext.getString(R.string.share_url) +"]");
			}else if(mData.get(position).mMessageInfo.typefile == MessageType.INVITE){
				holder.mContentTextView.setText("["+ mContext.getString(R.string.invite) +"]");
			}else if(mData.get(position).mMessageInfo.typefile == MessageType.VIDEO){
				holder.mContentTextView.setText("["+ mContext.getString(R.string.video) +"]");
			}
		}else{
