package com.coolwin.XYT.control;
...
import com.coolwin.XYT.action.AudioRecorderAction.RecorderListener;
...
//	private static String mAudioName = "temp_audio.mp3";
	...
	public void showDg() {
		...
		startLayout = (LinearLayout) dialog.findViewById(R.id.start);
		endLayout = (LinearLayout) dialog.findViewById(R.id.end);
		try {
	...
	// 录音结束,关闭dialog 效果
	public void cancelDg(boolean isCancel) {
		if(isCancel){
	...
	private void setDialogImg(double amplitude) {
		...
		} else if (index * 12 <= amplitude && amplitude < index * 13) {
			mDialogBackground.setImageResource(R.drawable.amp7);
		}
	}
