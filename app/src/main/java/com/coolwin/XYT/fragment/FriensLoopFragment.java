...
import com.coolwin.XYT.widget.MyPullToRefreshListView.OnChangeStateListener;
 ...
 *
 */
public class FriensLoopFragment extends Fragment implements OnTouchListener, OnChangeStateListener, OnPageChangeListener,
        OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		bbs = (Bbs) mContext.getIntent().getSerializableExtra("bbs");
		isvisitors =  mContext.getIntent().getBooleanExtra("isvisitors",false);
		if(bbs!=null){
			bid = bbs.id;
		}
		mMetric = new DisplayMetrics();
		mContext.getWindowManager().getDefaultDisplay().getMetrics(mMetric);
...
//		sendBroadcast(new Intent(GlobalParam.ACTION_HIDE_NEW_FRIENDS_LOOP));
	...
	private void initCompent(){
		mainBottom = (LinearLayout)mView.findViewById(R.id.main_bottom);
		if(isvisitors){
...
//		mRightTextBtn.setOnClickListener(this);
		...
		mBottomLayout = (RelativeLayout)mView.findViewById(R.id.bottom_menu);
		mBottomLayout.setVisibility(View.GONE);
		mTabs = new Button[5];
		mTabs[0] = (Button) mView.findViewById(R.id.live);
		mTabs[1] = (Button) mView.findViewById(R.id.qiye);
		mTabs[2] = (Button) mView.findViewById(R.id.btn_zhuye);
		mTabs[3] = (Button) mView.findViewById(R.id.weishan);
		mTabs[4] = (Button) mView.findViewById(R.id.huodong);
		mTabs[2].setSelected(true);
		//mPicBtn = (ImageView)findViewById(R.id.pic);
		//mPicBtn.setOnClickListener(this);
		mBottomMenuLayout = (LinearLayout)mView.findViewById(R.id.comment_bottom_menu);
...
//					break;
				case AbsListView.OnScrollListener.SCROLL_STATE_IDLE: //加载更多
...
//				}
		...
		mCommentEdit = (EditText)mView.findViewById(R.id.edit);
		mCommentEdit.setOnFocusChangeListener(sendTextFocusChangeListener);
		mCommentEdit.setOnClickListener(sendTextClickListener);


		mViewPager = (ViewPager) mView.findViewById(R.id.imagepager);
		mViewPager.setOnPageChangeListener(this);
		mLayoutCircle = (LinearLayout) mView.findViewById(R.id.circlelayout);
		mEmotionLayout = (RelativeLayout) mView.findViewById(R.id.emotionlayout);
			...
			public void run() {
				try {
					if (loadType == GlobalParam.LIST_LOAD_FIRST) {
						IMCommon.sendMsg(mHandler, BASE_SHOW_PROGRESS_DIALOG,
...
//					}
					...
					e.printStackTrace();
					IMCommon.sendMsg(mHandler,BASE_MSG_NETWORK_ERROR,
	...
	private void originalImage(Intent data) {
		...
		if (uri != null) {
			...
			if (!TextUtils.isEmpty(uri.getAuthority())) {
				...
				Intent intent = new Intent(mContext,FriensLoopFragment.class);
...
//				}
	...
	private void showDialog(){
		...
		builder.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				huiyuanshenjiIntent.putExtra("url","http://shop.wei20.cn/gouwu/wish3d/member_hy.shtml?id="+mlogin.ypId+"&k=%E7%89%B9%E6%9D%83%E4%BC%9A%E5%91%98&token="+mlogin.token+"&lbs="+ MainActivity.lbs);
		...
		new Thread(){
			public void run() {
				try {
					...
					IMCommon.sendMsg(mHandler, BASE_SHOW_PROGRESS_DIALOG, mContext.getResources().getString(R.string.send_request));
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_CLEAR_LISTENER_DATA:
				...
				if ((msg.arg1 == GlobalParam.LIST_LOAD_FIRST
						...
						|| msg.arg1 == GlobalParam.LIST_LOAD_UPDATE)
			...
			case GlobalParam.MSG_ZAN_STATUS:
				...
				if(zanResult.friendsLoopitem!=null){
					if(mDataList!=null && mDataList.size()>=mPosition){
						mDataList.get(mPosition).replylist = zanResult.friendsLoopitem.replylist;
						mDataList.get(mPosition).praiselist = zanResult.friendsLoopitem.praiselist;
					}
				}
	...
	public void onDestroy() {
		...
		mContext.sendBroadcast(new Intent(GlobalParam.ACTION_HIDE_NEW_FRIENDS_LOOP));
		mContext.unregisterReceiver(Receiver);
		super.onDestroy();
	}
