...
public class WriteUserInfoActivity extends BaseActivity implements OnClickListener {
	...
	private void initComponent(){
		...
		if(mType == GlobleType.COMPLETE_SEX){
			...
			mNameText.setPadding(10,10,10,10);
			mNameText.setLayoutParams(param);
			mNameText.setHint("公司主页");
		}else if(mType == GlobleType.COMPLETE_HANGYE){
			titileTextView.setText("行业");//
			mNameText.setHint("行业");
		}else if(mType == GlobleType.COMPLETE_DIANHUA){
			titileTextView.setText("电话");//
			mNameText.setHint("电话");
			mNameText.setInputType(InputType.TYPE_CLASS_PHONE);
		}else if(mType == GlobleType.COMPLETE_KEGONG){
			titileTextView.setText("可供");//
			LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, FeatureFunction.dip2px(mContext,120));
			int margin = FeatureFunction.dip2px(mContext, 10);
			param.setMargins(margin, margin,margin, 0);
			mNameText.setGravity(Gravity.TOP| Gravity.LEFT);
			mNameText.setPadding(10,10,10,10);
			mNameText.setLayoutParams(param);
			mNameText.setHint("可供");
		}else if(mType == GlobleType.COMPLETE_XUQIU){
			titileTextView.setText("需求");//
			LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, FeatureFunction.dip2px(mContext,120));
			int margin = FeatureFunction.dip2px(mContext, 10);
			param.setMargins(margin, margin,margin, 0);
			mNameText.setGravity(Gravity.TOP| Gravity.LEFT);
			mNameText.setPadding(10,10,10,10);
			mNameText.setLayoutParams(param);
			mNameText.setHint("需求");
		}else if(mType == GlobleType.COMPLETE_GONGSIDIZI){
			titileTextView.setText("公司地址");//
			LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, FeatureFunction.dip2px(mContext,120));
			int margin = FeatureFunction.dip2px(mContext, 10);
			param.setMargins(margin, margin,margin, 0);
			mNameText.setGravity(Gravity.TOP| Gravity.LEFT);
			mNameText.setPadding(10,10,10,10);
			mNameText.setLayoutParams(param);
			mNameText.setHint("公司地址");
		}else if(mType == GlobleType.COMPLETE_SUOZAIGONGSI){
			titileTextView.setText("所在公司");//
			LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, FeatureFunction.dip2px(mContext,120));
			int margin = FeatureFunction.dip2px(mContext, 10);
			param.setMargins(margin, margin,margin, 0);
			mNameText.setGravity(Gravity.TOP| Gravity.LEFT);
			mNameText.setPadding(10,10,10,10);
			mNameText.setLayoutParams(param);
			mNameText.setHint("所在公司");
		}else if(mType == GlobleType.COMPLETE_ZHIWEI){
			titileTextView.setText("职位");//
			mNameText.setHint("职位");
		}
	...
	public void onClick(View v) {
		...
		case R.id.right_btn:
			...
			if(mType == GlobleType.COMPLETE_NICKNAME){
				...
				intent.putExtra("companywebsite", content);
			}else if(mType == GlobleType.COMPLETE_HANGYE){
				intent.putExtra("industry", content);
			}else if(mType == GlobleType.COMPLETE_SUOZAIGONGSI){
				intent.putExtra("company", content);
			}else if(mType == GlobleType.COMPLETE_GONGSIDIZI){
				intent.putExtra("companyaddress", content);
			}else if(mType == GlobleType.COMPLETE_ZHIWEI){
				intent.putExtra("job", content);
			}else if(mType == GlobleType.COMPLETE_DIANHUA){
				intent.putExtra("telephone", content);
			}else if(mType == GlobleType.COMPLETE_KEGONG){
				intent.putExtra("provide", content);
			}else if(mType == GlobleType.COMPLETE_XUQIU){
				intent.putExtra("demand", content);
			}
