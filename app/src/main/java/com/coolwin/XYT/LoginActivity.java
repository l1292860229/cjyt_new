...
public class LoginActivity extends BaseActivity implements OnClickListener {
	...
	private Handler mHandler=new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_LOAD_ERROR:
				hideProgressDialog();
				IMJiaState error_Detail = (IMJiaState)msg.obj;
				if(error_Detail != null && !error_Detail.errorMsg.equals("")){
					Toast.makeText(mContext,error_Detail.errorMsg, Toast.LENGTH_LONG).show();
				}else{
	 ...
	 * @param password
	 */
	private void getLogin(final String loginUserName, final String password) {
		new Thread() {
			@Override
			public void run() {
				try {
					LoginResult result = getIMInfo().getLogin(loginUserName, password);
					if (result != null) {
						if (result.mState != null && result.mState.code == 0) {
							if (result.mLogin != null) {
								...
								IMCommon.setPhone(result.mLogin.phone);
							}
							Editor editor = mPreferences.edit();
							editor.putBoolean(IMCommon.ISREMENBER, true);
							editor.putString(IMCommon.USERNAME, loginUserName);
							...
							if(BMapApiApp.getContryList() == null
								...
								BMapApiApp.setContryList(getIMInfo().getCityAndContryUser());
							}
