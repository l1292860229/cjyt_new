						...
						public void run() {
							try {
								if(type == GlobleType.BLOCKLISTACTIVITY_BLOCK_TYPE){
									mUser = IMCommon.getIMInfo().getBlockList(/*currentPage*/);
								}
	...
	private void cancelBlock(final String fuid, final int isFollow){
		if(IMCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						IMJiaState state = null;
						if(mType == 1){
							state = IMCommon.getIMInfo().addfocus(fuid/*,isFollow*/);
						}else{
							state= IMCommon.getIMInfo().cancelBlock(fuid);
						}
