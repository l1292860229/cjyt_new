...
import com.coolwin.XYT.adapter.BlockListAdapter;
					...
					new Thread(){
						public void run() {
							try {
								...
								if(mType == 1){
									mUser = IMCommon.getIMInfo().huoyueList(currentPage,mMeetingId);
								}else{
									mUser = IMCommon.getIMInfo().meetingApplyList(currentPage,mMeetingId);
								}
	...
	private void agreeApplyMeeting(final String fuid, final int pos){
		if(IMCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						IMJiaState state = IMCommon.getIMInfo().agreeApplyMeeting(mMeetingId,fuid);
						if(state!=null){
							state.positon = pos;
						}
	...
	private void disAgreeApplyMeeting(final String fuid, final int pos){
		if(IMCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						IMJiaState state = IMCommon.getIMInfo().disagreeApplyMeeting(mMeetingId, fuid);
						if(state!=null){
							state.positon = pos;
						}
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
	...
	private void removeMetUser(final String fuid, final int pos){
		if(IMCommon.verifyNetwork(mContext)){
			new Thread(){
				...
				public void run(){
					try {
						IMJiaState state = IMCommon.getIMInfo().removeMetUser(mMeetingId, fuid);
						if(state!=null){
							state.positon = pos;
						}
						mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_AGREE_APPLY_METTING:
				IMJiaState agreeStatus = (IMJiaState)msg.obj;
				if(agreeStatus == null){
			...
			case GlobalParam.MSG_DIS_AGREE_APPLY_METTING:
				IMJiaState disAgreeStatus = (IMJiaState)msg.obj;
